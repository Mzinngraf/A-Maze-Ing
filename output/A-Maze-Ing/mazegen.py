...
class MazeGenerator:
    ...
    def pattern(self):
        if self.height >= 7 and self.width >= 9:
            ...
            placex = (self.width - 7) // 2
            placey = (self.height - 5) // 2
            self.fortytwo = set((i + placex, j + placey) for (i, j) in fortytwopattern)
            for (x, y) in self.fortytwo:
                self.basegrid.cells[y][x] = 15
            if (x + 1, y) in fortytwopattern:
                self.basegrid.cells[y][x] -= 2
                self.basegrid.cells[y][x + 1] -= 8
            if (x, y + 1) in fortytwopattern:
                self.basegrid.cells[y][x] -= 4
                self.basegrid.cells[y + 1][x] -= 1
        else:
